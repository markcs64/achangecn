var PPEditor = function( instanceName)
{
    ...
    this.Width          = '100%' ;
    this.Height         = '500' ;
    this.LinkPath       = '/vipuser/edit/skin/';
    this.SkinPath       = '/vipuser/edit/skin/' ;
    ...
    this.AllowNumber    = 50000;
	this.CurrenNumber   = 0;
	this.EditorBGColor  = "#E0F4FF";
	this.EditorBorder	= "1px solid #0275BF";
	this.EditorOVColor  = "#91D4FF";
    this.ToolbarSets    = [['Bold','Italic','Underline','StrikeThrough'],['SubScript','SuperScript'],['InsertOrderedList','InsertUnOrderedList'],['JustifyLeft','JustifyCenter','JustifyRight'],['CreateLink','UnLink'],['InsertImage'],['TextColor','BGColor'],'/',['FontName','FontSize'],'/',['Source','Preview'],['WordCount']] ;
	...
	this.CreateLinkMsg  = '<div class="editor_line_left"><br /><fieldset><legend>{title}</legend><br />&nbsp;&nbsp;地址：<input type="text" id="_link_url" size="30" value="{url}" class="editor_input_main"><br /><br />&nbsp;&nbsp;例如：http://www.paipai.com/<br /><br /></fieldset><br /><fieldset><legend>其他选项</legend><br />&nbsp;&nbsp;在新窗口打开:<input id="_link_target" type="checkbox" value="1" {target}><br /><br /></fieldset></div><br /><div class="editor_line_center"><input type="button" value=" 确定 " onclick="DoInsertLink()">&nbsp;&nbsp; <input type="button" value=" 取消 " onclick="CloseDialog()"></div>';
...
function CountWord(){
    ...
    if (count_default) {
        oEditor.CurrenNumber=count_default.innerHTML = oEditor.Editor.document.body.innerHTML.replace(/[^\x00-\xff]/g,"**").replace(/\s/g,"*").length;
    }
    if (count_source) {
        oEditor.CurrenNumber=count_source.innerHTML = oEditor.TextObj.value.replace(/[^\x00-\xff]/g,"**").replace(/\s/g,"*").length;
    }
...
function IconClick(action,text){
    ...
    if(action=='Source'){
		...
		alert("您当前已输入[" + oEditor.CurrenNumber + "]字符。您还可以再输入["+(oEditor.AllowNumber-oEditor.CurrenNumber)+"]字符！");
	} else{
		oEditor.ReSelect();
...
PPEditor.prototype.Preview = function()
{
	if(this.CurrenNumber>this.AllowNumber){
		alert("您当前已输入[" + this.CurrenNumber + "]字符，超出限制["+(this.CurrenNumber-this.AllowNumber)+"]字符！请删除部分内容后重试，谢谢！");
		return false;
	}
...
PPEXHtml._AppendNode = function( xmlNode, htmlNode )
{
	switch ( htmlNode.nodeType )
	{
		case 1 :	
			...
			break ;
		case 3 :
			this._AppendTextNode( xmlNode, htmlNode.nodeValue) ;
			break ;
		case 8 :
